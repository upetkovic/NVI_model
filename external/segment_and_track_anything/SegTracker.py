...
# Add the parent of sam/, so "sam" is importable
sam_parent = os.path.abspath(os.path.join(os.path.dirname(__file__), "sam/.."))
sys.path.append(sam_parent)

#from segment_and_track_anything.sam
...
class SegTracker():
    ...
    def detect_only(self, origin_frame: np.ndarray, grounding_caption, box_threshold, text_threshold, box_size_threshold=1, reset_image=False, bbox_search = 0):
        '''
    ...
    def detect_and_seg_mod(self, origin_frame: np.ndarray, grounding_caption, box_threshold, text_threshold, box_size_threshold=1, reset_image=False, bbox_search = 0):
        '''
        Using Grounding-DINO to detect object acc Text-prompts
        Retrun:
            refined_merged_mask: numpy array (h, w)
        ...
        ### find bbox with highest iou

        #boxes = [boxes[0]]
        if bbox_search:
            iou_min = 0
            for i, bbox in enumerate(boxes):
                iou = get_iou(bbox, bbox_search)
                if iou > iou_min:
                    ind_max = i
                    iou_min = iou
        ...
        return refined_merged_mask, annotated_frame, boxes
