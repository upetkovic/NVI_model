...
OUTPUT_FOLDER_PATH = f"/media/uros/b497d77f-967a-48ec-aea0-5c352966318c/new_data/project31/data/NI/depth/real_depth{frame_reduce_factor}"
OUTPUT_FOLDER_PATH = "data/outputs/videos/depth"
...
def extract_depth_from_video(video_path, output_folder, frame_reduce_factor=1):
    ...
    # Define local paths where you want to cache checkpoints
    local_checkpoint_dir = "./checkpoints"  # or wherever you want
    os.makedirs(local_checkpoint_dir, exist_ok=True)

    local_config_path = os.path.join(local_checkpoint_dir, f"{backbone_name}_{HEAD_DATASET}_{HEAD_TYPE}_config.py")
    local_checkpoint_path = os.path.join(local_checkpoint_dir, f"{backbone_name}_{HEAD_DATASET}_{HEAD_TYPE}_head.pth")

    # Download config if not existing
    ...
    # Download checkpoint if not existing
    if not os.path.exists(local_checkpoint_path):
        # manual download
        import urllib.request
        urllib.request.urlretrieve(head_checkpoint_url, local_checkpoint_path)

    # Load checkpoint
    load_checkpoint(model, local_checkpoint_path, map_location="cpu")
    ...
    output_path_real = os.path.join(output_folder, video_id.split('.')[0] + ".npy")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Extract depth from video")
    parser.add_argument('--video_path', type=str, required=True, help='Path to the input video file')
    parser.add_argument('--output_folder', type=str, required=True, help='Folder to save the depth output')
...
if __name__ == "__main__":
    ...
    extract_depth_from_video(args.video_path, args.output_folder)